...
- 최신 병합 기준: `main`
- 최신 완료 기능: HTTP Outbox Broker Adapter
- 현재 릴리스 후보: `v0.6.0`
- 아직 미완료: 운영자 manual review 화면, Kafka/RabbitMQ/SQS adapter, consumer idempotency, operator/admin token 분리, pruning 실행 이력, external alert channel, 승인 워크플로우, PostgreSQL scenario test, GitHub Wiki 동기화, Testcontainers 강제 실행 정책
