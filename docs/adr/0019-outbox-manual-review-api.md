...
`manual review 조회와 requeue API 제공`을 선택한다. requeue 행위는 금융/핀테크 운영에서 감사 대상이므로 operator와 reason을 받아 별도 감사 이력으로 남긴다.
...
| 감사 이력 | `operation_outbox_requeue_audits`에 operator, reason, requeuedAt 저장 |
...
- 인증된 사용자와 operator가 아직 연결되지 않는다.
