# Architecture Decision Records

이 폴더는 `ai-repo`의 중요한 기술, 운영, 도메인 결정을 기록합니다.

ADR은 이 저장소에서 중요한 결정의 source of truth입니다. README와 Wiki는 ADR을 요약하거나 연결할 수 있지만, 결정 자체를 대체하지 않습니다.

## 작성 원칙

- 결정이 필요한 배경과 제약을 먼저 설명합니다.
- 최소 2개 이상의 선택지를 비교합니다.
- 선택한 결정뿐 아니라 포기한 선택지의 비용도 기록합니다.
- 결정의 결과와 후속 작업을 명시합니다.
- 결정이 바뀌면 기존 ADR을 수정하기보다 새 ADR로 대체 관계를 남깁니다.

## 목록

| 번호 | 제목 | 상태 |
| --- | --- | --- |
| ADR-0001 | Documentation Source of Truth | Accepted |
| ADR-0002 | Test Strategy | Accepted |
| ADR-0003 | Java, Spring Boot, Gradle Baseline | Accepted |
| ADR-0004 | Gradle Wrapper for Java 25 Runtime | Accepted |
| ADR-0005 | Member Wallet Account Query Policy | Accepted |
| ADR-0006 | Charge Transfer Idempotency Policy | Accepted |
| ADR-0007 | Ledger Audit Log Boundary | Accepted |
| ADR-0008 | PostgreSQL Persistence Profile | Accepted |
| ADR-0009 | PostgreSQL Runtime Verification | Accepted |
| ADR-0010 | Flyway Schema Migrations | Accepted |
| ADR-0011 | PostgreSQL Balance Row Locking | Accepted |
| ADR-0012 | PostgreSQL Lock Timeout Policy | Accepted |
| ADR-0013 | Operation Step Log Before Outbox and Saga | Accepted |
| ADR-0014 | Transactional Outbox Boundary | Accepted |
| ADR-0015 | Outbox Relay State | Accepted |
| ADR-0016 | Outbox Claiming and Retry Policy | Accepted |
| ADR-0017 | Outbox Processing Lease Recovery | Accepted |
| ADR-0018 | Outbox Max Attempt and Manual Review | Accepted |
| ADR-0019 | Outbox Manual Review API | Accepted |
| ADR-0020 | Outbox Requeue Audit Trail | Accepted |
| ADR-0021 | Release Version Baseline | Accepted |
| ADR-0022 | Scenario-Based Test Pipeline | Accepted |
| ADR-0023 | Validation Summary Hardening | Accepted |
| ADR-0024 | React User Frontend MVP | Accepted |
| ADR-0025 | Frontend E2E Test Pipeline | Accepted |
| ADR-0026 | Frontend Component Test Pipeline | Accepted |
| ADR-0027 | Outbox Publisher Port | Accepted |
| ADR-0028 | Admin API Authz Boundary | Accepted |
| ADR-0029 | Outbox Relay Scheduler | Accepted |
| ADR-0030 | Outbox Relay Run Monitoring | Accepted |
| ADR-0031 | Admin API Access Audit | Accepted |
| ADR-0032 | Operational Log Pruning | Accepted |
| ADR-0033 | Outbox Relay Health Metrics and Alert | Accepted |
| ADR-0034 | Spring Security Role Model | Accepted |
| ADR-0035 | HTTP Outbox Broker Adapter | Accepted |
| ADR-0036 | PostgreSQL Scenario Testcontainers CI Gate | Accepted |
| ADR-0037 | Operator/Admin Token Split | Accepted |
| ADR-0038 | Requeue Approval Workflow | Accepted |
| ADR-0039 | Dev Rules Automatic Sync Check | Accepted |
| ADR-0040 | Direct Requeue API Deprecation | Accepted |
| ADR-0041 | Requeue State Transition Atomicity | Accepted |
| ADR-0042 | Outbox Claim Guarded Result Update | Accepted |
| ADR-0043 | Broker and Consumer Idempotency Contract | Accepted |
| ADR-0044 | Consumer Processed Event Dedupe Store | Accepted |
| ADR-0045 | HTTP Outbox Consumer Adapter | Accepted |
| ADR-0046 | HTTP Publish Consume Loop Verification | Accepted |
| ADR-0047 | Consumer Monitoring Admin API | Accepted |
| ADR-0048 | Consumer Processed Event Pruning | Accepted |
| ADR-0049 | Consumer Duplicate Spike Alert | Accepted |
| ADR-0050 | Consumer Duplicate Time Bucket Metric | Accepted |
| ADR-0051 | Consumer Delivery Metric Pruning | Accepted |
| ADR-0052 | Operational Alert Record Channel | Accepted |
| ADR-0053 | Operational Alert Suppression and Pruning | Accepted |
| ADR-0054 | Slack Webhook Operational Alert Publisher | Accepted |
| ADR-0055 | Admin API Path Matching Hardening | Accepted |
| ADR-0056 | End-User JWT Authentication and Wallet Ownership | Accepted |
| ADR-0057 | Frontend Member Login and Bearer Token with 401 Re-issue | Accepted |
| ADR-0058 | Audit Events and Operation Log Authorization | Accepted |
| ADR-0059 | 로컬 k8s 배포와 관측(모니터링·로그·GitOps) 스택 | Accepted |
| ADR-0060 | Audit Event–Wallet Mapping 명시화 | Accepted |
| ADR-0061 | k8s 자격증명 Secret 주입 전환 | Accepted |
| ADR-0062 | 배포 프로파일에서 기본 자격증명 fail-fast | Accepted |
| ADR-0063 | Application Layer Spring Annotation Policy | Accepted |
| ADR-0064 | JDBC Persistence Adapter Decomposition | Accepted |
| ADR-0065 | Broker Consumer Endpoint Authentication | Accepted |
| ADR-0066 | 학습용 opt-in ELK 로깅 스택 | Accepted |
| ADR-0067 | Outbox Publisher Deployment Wiring | Accepted |
| ADR-0068 | Deployed Profile Persistence Guard | Accepted |
| ADR-0069 | Transfer Idempotency Resolution Ordering | Accepted |
| ADR-0071 | Hot Query Indexes and Batched Outbox Claim | Accepted |
| ADR-0072 | Wallet-Scoped Idempotency Keys | Accepted |
| ADR-0073 | Stale Cleanup Artifacts | Accepted |
| ADR-0070 | Input and Response Boundary Hardening | Accepted |
