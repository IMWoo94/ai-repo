...
현재 기능 흐름은 잔액/거래내역 조회, 회원/지갑 계정, 충전/송금 1차 API를 제공합니다.
...
충전/송금은 Issue #5 기준으로 확장 중이며, 1차 범위에서는 `KRW` 단일 통화와 멱등키를 사용합니다.
...
- `v0.3.0`: PostgreSQL 저장소, 원장 모델, 멱등 키 영속화
- `v0.4.0`: 원장 모델, 감사 로그
- `v0.5.0`: 정산 배치, 재처리, 장애 보상
